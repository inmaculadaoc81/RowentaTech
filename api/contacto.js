...
export default async function handler(req,res){
  ...
  try{
    ...
    const subject=`Nueva consulta RowentaTech - repararrobotaspirador.com.es | ${modelo||'Robot aspirador Rowenta'}`;
    const html=`<div style="font-family:Arial,sans-serif;line-height:1.6;color:#1a1c1e"><h2 style="color:#0ea89f">Nueva consulta desde RowentaTech</h2><p><strong>Nombre:</strong> ${safe.nombre}</p><p><strong>Teléfono:</strong> ${safe.telefono}</p><p><strong>Email:</strong> ${safe.email}</p><p><strong>Modelo Rowenta:</strong> ${safe.modelo}</p><p><strong>Descripción de la avería:</strong><br>${safe.mensaje}</p><hr><p style="font-size:12px;color:#5b6570">Enviado desde el formulario de repararrobotaspirador.com.es</p></div>`;
    const mime=[`From: RowentaTech <${from}>`,`To: ${to}`,`Reply-To: ${email}`,`Subject: ${encodeHeader(subject)}`,'MIME-Version: 1.0','Content-Type: text/html; charset="UTF-8"','Content-Transfer-Encoding: 8bit','',html].join('\r\n');
